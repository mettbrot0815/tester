#!/usr/bin/env zsh
# =============================================================================
#  install.zsh  –  Ubuntu WSL2  ·  llama.cpp + Hermes + Goose + OpenCode + AutoAgent + OpenClaude + WebUI
...
# =============================================================================
#  1. HuggingFace token – SAFE EXTRACTION (no sed/grep parsing of .zshrc)
# =============================================================================
_HF_ENV="${HF_TOKEN:-}"
HF_TOKEN=""
if [[ -n "$_HF_ENV" ]]; then
    ...
    # Safely source .zshrc in a subshell to extract HF_TOKEN without sed/grep injection
    if [[ -f "${HOME}/.zshrc" ]]; then
        extracted=$(zsh -c "source ${HOME}/.zshrc 2>/dev/null && print -rn \"\$HF_TOKEN\"" || true)
        if [[ -n "$extracted" ]]; then
            HF_TOKEN="$extracted"
...
if [[ -n "$_GH_ENV" ]]; then
    ...
    # Safe extraction from .zshrc (subshell)
    if [[ -f "${HOME}/.zshrc" ]]; then
        extracted=$(zsh -c "source ${HOME}/.zshrc 2>/dev/null && print -rn \"\$GITHUB_TOKEN\"" || true)
        if [[ -n "$extracted" ]]; then
...
if [[ -z "$_SMO" ]]; then
    ...
    # Install Zsh tools and configure .zshrc
    ...
    if ! grep -qF "$ZSHRC_MARKER" "${HOME}/.zshrc" 2>/dev/null; then
        ...
        if [[ -t 0 ]]; then
            ...
            configure_zsh="y"  # Default to yes in non-interactive
        fi
...
# Use associative array for model data
typeset -A MODELS
MODELS=(
...
find_llama_server() {
    ...
    local found
    # Use Zsh glob qualifier for efficient finding
...
select_optional_components() {
    ...
    # Parse the quoted output using Zsh array expansion
...
# Use Zsh parameter expansion instead of envsubst
template_content=$(<"${LAUNCH_SCRIPT}.template")
print -r ${(e)template_content} > "$LAUNCH_SCRIPT"
...
# Override clear to show summary
